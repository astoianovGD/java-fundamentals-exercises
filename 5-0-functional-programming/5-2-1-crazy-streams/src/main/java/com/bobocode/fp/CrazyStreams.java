...
public class CrazyStreams {
    ...
    public List<Account> findAccountsByBirthdayMonth(Month birthdayMonth) {
          return accounts.stream().filter(x -> x.getBirthday().getMonth().equals(birthdayMonth)).toList();
    }
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
         return accounts.stream().collect(Collectors.partitioningBy(x -> x.getSex().equals(Sex.MALE)));
    }
    ...
    public int getNumOfLettersInFirstAndLastNames() {
        return accounts.stream().mapToInt(x -> x.getFirstName().length() + x.getLastName().length()).sum();
    }
    ...
    public boolean containsAccountWithEmailDomain(String emailDomain) {
        return !accounts.stream().filter(x -> x.getEmail().contains(emailDomain)).toList().isEmpty();
    }
     ...
     */
        public Map<String, BigDecimal> collectBalancesByEmailForAccountsCreatedOn(int year) {
            return accounts.stream().filter(x -> x.getCreationDate().getYear() == year)
    ...
    public Map<String, Set<String>> groupFirstNamesByLastNames() {
        return accounts.stream().collect(Collectors.groupingBy(
                Account::getLastName,
                Collectors.mapping(
                        Account::getFirstName, // 1. Що дістаємо з акаунта
                        Collectors.toSet()     // 2. Куди збираємо результат
                )
        ));
    }
